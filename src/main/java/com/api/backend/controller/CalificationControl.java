...
public class CalificationControl {
    ...
    public List<CalificationModel> getCalificationsListByClass(@CookieValue("token") String token, @RequestHeader("ClassId") int Id) {
    ...
    public List<CalificationModel> getCalificationsByEmail(@CookieValue("token") String token) {
    ...
    public ResponseEntity<Boolean> CreateUpdateCalifications(@CookieValue("token") String token, @RequestBody List<CalificationModel> califications) {
        if(jwtService.ValidateTokenAdminTeacher(token)){
            try {
