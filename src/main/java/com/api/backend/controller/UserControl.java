...
public class UserControl {
    ...
    @GetMapping("/getAll")
    public List<UserModel> obtainUserList(@CookieValue("token") String token) {
    ...
    public List<UserModel> obtainTeacherList(@CookieValue("token") String token) {
        if (jwtService.ValidateTokenAdmin(token)) {
    ...
    public List<UserModel> obtainParentsList(@CookieValue("token") String token) {
        if (jwtService.ValidateTokenAdmin(token)) {
    ...
    public List<UserModel> obtainStudentsList(@CookieValue("token") String token) {
        if (jwtService.ValidateTokenAdmin(token)) {
    ...
    public ResponseEntity<UserModel> obtainUserByEmail(@CookieValue("token") String token) {
    ...
    public ResponseEntity<Boolean> validateUser(@CookieValue("token") String token) {
        String email = jwtService.extractEmailFromToken(token);
    ...
    public ResponseEntity<Boolean> validateAdmin(@CookieValue("token") String token) {
        String email = jwtService.extractEmailFromToken(token);
        if (email == null || email.isEmpty()) {
            return ResponseEntity.badRequest().body(false);
        }
    ...
    public ResponseEntity<Boolean> validateTeachersAdmins(@CookieValue("token") String token) {
        String email = jwtService.extractEmailFromToken(token);
        if (email == null || email.isEmpty()) {
            return ResponseEntity.badRequest().body(false);
        }
    ...
    public UserModel updateUserByEmail(@CookieValue("token") String token, @RequestBody UserModel user) {
        String email = jwtService.extractEmailFromToken(token);
        return userService.updateUserByEmail(email, user);
    }

    @GetMapping("/infoCompleteByEmail")
    public ResponseEntity<Boolean> infoCompleteByEmail(@CookieValue("token") String token) {
        String email = jwtService.extractEmailFromToken(token);
        return ResponseEntity.ok(userService.InfoCompleteByEmail(email));
    }

    @GetMapping("/getInfoByEmail")
    public AuxiliarUserModel getInfoByEmail(@CookieValue("token") String token) {
        String email = jwtService.extractEmailFromToken(token);
        return userService.getInfoByEmail(email);
    }

    @GetMapping("/listUserInfo")
    public List<AuxiliarUserModel> getUsersWithSpecificFields(@CookieValue("token") String token) {
        if (jwtService.ValidateTokenAdmin(token)) {
